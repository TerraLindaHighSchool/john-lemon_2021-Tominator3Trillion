...
public class PlayerMovement : MonoBehaviour, IPunObservable
{
    ...
    void Update() {
        ...
        //if D is pressed die
        if(view.IsMine && !isAlive && Input.GetKeyDown(KeyCode.J)) {
            Die();
        }
        ...
        // if touching another non hunter upsideDownCharged = true, check every 25 seconds
        if(Mathf.Floor(Time.timeSinceLevelLoad) % 25f == 0f && view.IsMine && !upsideDown && !upsideDownCharged && !isHunter && isAlive) {
            Collider[] hitColliders = Physics.OverlapSphere(transform.position, 3f);
            foreach(Collider col in hitColliders) {
                if(col.gameObject.GetComponent<PlayerMovement>() && !col.gameObject.GetComponent<PlayerMovement>().isHunter && col.gameObject.GetComponent<PlayerMovement>().isAlive) {
